...
describe 'Импорт YML: проверка доступа страницы владельцу (бесплатный пакет)',
         feature: 'Импорт YML: проверка доступа страницы владельцу (бесплатный пакет)' do
  before(:all) do
  ...
  context 'когда роль - Владелец' do
    ...
    context 'когда компания с бесплатным пакетом' do
      before(:all) do
      ...
      include_examples 'проверка таба "Расширенные настройки" для владельца (бесплатный пакет)'
    end
  end
end
